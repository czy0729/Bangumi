...
<p align="center">
  <img alt="Version: 3.12.0" src="https://img.shields.io/badge/Version-3.12.0-0366d6" />
...
- [ ] [条目] 加快进入条目页面时, 对条目类型的判断, 页面更快显示正确的结构
- [ ] [条目] 更合理地区分书籍 (漫画 | 小说)
- [x] 升级底层框架版本 (0.59 -> 0.62), 默认修复部分现有问题
- [x] 因框架新版本, android/pageViewer 废弃, 所有旧 Tabs 替换新库实现
- [ ] 条目数据迁移出 APP, 缩小包体积
- [ ] 列表里自己的头像不走快照, 保证最新
- [x] 屏蔽柠萌瞬间相关, 下版本完全移除
- [x] 现在不会重复进入参数完全相同的页面
- [ ] 请求中的转圈可以手动关闭, 避免请求超时无法操作
- [ ] 优化了漫画阅读器的图片加载顺序

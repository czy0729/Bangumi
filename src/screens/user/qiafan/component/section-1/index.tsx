...
import { useGithubStats } from '../../github'
import { WEEKS_RECENT_COUNT } from './ds'
import CommitsOverTime from './commits-over-time'
import Contributions from './contributions'

function Section1() {
  const { commits, releases, additions, deletions, login, avatarUrl, authorWeeks, weeks } =
    useGithubStats()

  const cardWidth = _.window.width - _._wind * 2

  /** 作者卡片只显示最近 3 个月, 固定切片引用让 BarChart 的 useMemo 命中缓存 */
  const authorRecentWeeks = useMemo(() => authorWeeks.slice(-WEEKS_RECENT_COUNT), [authorWeeks])
  ...
  return (
    <>
      <Text {...FONT_BASE}>
        ...
        <Text {...FONT_STRONG}>{commits}</Text> 次代码提交，超过百万行代码的增改，
        <Text {...FONT_STRONG}>{releases}</Text> 多次版本发布，用爱发电超过数万小时。
      </Text>
      <View style={_.mt.md}>
        <Contributions
          width={cardWidth}
          weeks={authorRecentWeeks}
          login={login}
          avatarUrl={avatarUrl}
          commits={commits}
          additions={additions}
      ...
      <View style={_.mt.md}>
        <CommitsOverTime width={cardWidth} weeks={weeks} />
      </View>

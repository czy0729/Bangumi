 ...
 * @Last Modified time: 2025-09-22 14:48:06
 */
...
export function getThumbs(subjectId: SubjectId, isADV: boolean, thumb?: boolean) {
  if (isADV) {
    ...
    return new Array(length).fill('').map((_item, index) => CDN_ADV(subjectId, index, thumb))
  }

  const length = otaStore.game(subjectId)?.l
  if (typeof length !== 'number' || length <= 0) return []

  return new Array(length).fill('').map((_item, index) => CDN_GAME(subjectId, index, thumb))
}

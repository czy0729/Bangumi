/*
 * @Author: czy0729
 * @Date: 2025-12-31 21:06:09
 * @Last Modified by: czy0729
 * @Last Modified time: 2026-01-06 07:36:50
 */
import { HOST } from '@constants'

export const COMPONENT = 'DiscoveryUsers'

export const DATA = [
  {
    title: '2025 社区年度报告',
    url: 'https://bgm.ry.mk/report2025/?user=[USER_ID]',
    topic: `${HOST}/group/topic/445853`,
    name: '綿飴',
    userId: 'wataame'
  },
  {
    title: '2025 年度动画报告',
    url: 'https://report.mikuorz.top',
    topic: `${HOST}/group/topic/446062`,
    name: 'Mikuorz',
    userId: 'pereza'
  },
  {
    title: '2025 年度书籍报告',
    url: 'https://tongluoxia.tosuto.site',
    topic: `${HOST}/group/topic/446395`,
    name: '童洛夏',
    userId: 'tongluoxia'
  },
  {
    title: '巡礼地图',
    url: 'https://www.anitabi.cn/map',
    topic: `${HOST}/group/topic/376643`,
    name: '卜卜口',
    userId: 'itorr'
  },
  {
    title: 'Netabare',
    url: 'https://netaba.re',
    topic: `${HOST}/group/topic/346147`,
    name: '若卡',
    userId: 'ruocaled'
  },
  {
    title: '班谷米排名大王',
    url: 'https://bgmtier.sakuga.org',
    topic: `${HOST}/group/topic/447315`,
    name: 'ANNA',
    userId: 'annblack'
  },
  {
    title: 'VIB 数据月刊',
    path: 'VIB',
    topic: `${HOST}/group/qpz`,
    name: 'Jirehlov',
    userId: 'jirehlov'
  },
  {
    title: 'BMO:bmoji 合成大表情',
    url: `${HOST}/js/lib/bmo/gen_bmo.html`,
    topic: `${HOST}/group/bmoji`,
    name: '五行行行行行啊',
    userId: '572805'
  },
  {
    title: 'Bangumi 半月刊',
    path: 'BiWeekly',
    topic: `${HOST}/group/biweekly`,
    name: '他说nil没法调用IsNil',
    userId: 'neutrinoliu'
  }
] as const

...
const HOST_RAKUEN = `${HOST_CDN_STATIC}/Bangumi-Rakuen` as const

/** 超展开小组 CDN 自维护数据 */
export const CDN_RAKUEN = (topicId: Id, type: 'topic' | 'comment' = 'topic') => {
  return `${HOST_RAKUEN}/data/${type}/${getFolder(topicId)}/${topicId}.json`
}

/** 某用户的超展开 */
export const CDN_RAKUEN_USER_TOPICS = (userId: UserId) => {
  return `${HOST_RAKUEN}/data/user/${String(userId).slice(0, 1)}/${userId}.json`
}
